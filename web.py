...
global socket_send_queue
socket_send_queue = Queue(maxsize=10)

def socket_message_send_handler():
	while True:
		print("Ready to send from tx queue")
		message = socket_send_queue.get()
		print("Sent via tx queue")
		socketio.emit("asl_transcript", {"message": message}, to=None)

def send_asl_transcript(message):
	print(
		f"[send_asl_transcript] message={message}"
	)
	with app.app_context():
		print(f"[send_asl_transcript] Emitting asl_transcript: {message}")
		socket_send_queue.put(message)
		print("Added to tx queue " + str(socket_send_queue.qsize()))
...
def handle_connect():
	...
	connected_clients += 1
	print(f"[SOCKET] Client connected. Total clients: {connected_clients}")
	print(f"[SOCKET] Request remote address: {request.remote_addr}")
...
def handle_disconnect():
	...
	connected_clients -= 1
	print(f"[SOCKET] Client disconnected. Total clients: {connected_clients}")
